...
from pytorchCNNnew import MyCNN

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# モデルロード
model = MyCNN(num_classes=8).to(device)#同じ構造であることが必要
model.load_state_dict(torch.load("/workspace/checkpoints/final.pth", map_location=device))#ファイル指定として読み込み
model.eval()
